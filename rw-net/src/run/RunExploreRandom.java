 ...
 * @author mtirico
 */
public class RunExploreRandom {
    static int sim = 0 ;
    public static int run( int arg1,int arg2, int arg3,int arg4) throws IOException  {
//        System.out.println("ciao " + arg1);       Graph g = new SingleGraph(Integer.toString(args));      System.out.println(g.getId());
...
//		System.out.println("name file = " + nameFile) ;
		...
		System.out.println("sim: "+ sim++ + " finish " + nameFile + " " + new Date().toString() + " speed -> "  + t / (T/1000f) + " step/s , seeds -> "  + rws.getNumSeed()  +  ", nodes -> " + net.getNodeCount() 	 ) ; 

		if (storePng) 
			net.addAttribute("ui.screenshot",  pathPng ) ;
		sNet.close();
                
                Double[] ret = new Double[]{(double)sim,(double)net.getNodeCount()};
                System.out.println(ret[0]+ " "+ ret[1]);
//                return ret;
//                return ret;
                int num = net.getNodeCount();
//                return new int[]{sim,num};
                return num;
    }
}
